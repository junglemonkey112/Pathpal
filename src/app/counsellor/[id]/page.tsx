...
import { generateICS, getGoogleCalendarUrl, getAppleCalendarUrl, downloadICS, type CalendarEvent } from "@/utils/calendar";
...
export default function CounsellorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleConfirmBooking = async () => {
    ...
    // Build calendar URLs — no auto-download, user picks their preferred method
    setGoogleCalendarUrl(getGoogleCalendarUrl(calEvent));
    const ics = generateICS(calEvent);
    setIcsContent(ics);
    setAppleCalendarUrl(getAppleCalendarUrl(ics));
              ...
              {bookingConfirmed ? (
                <div className="py-4">
                  ...
                  <div className="space-y-2">
                    {/* Google Calendar — primary, works on all platforms */}
                    {googleCalendarUrl && (
                      <a
                        href={googleCalendarUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center justify-center gap-2 w-full py-2.5 bg-emerald-500 hover:bg-emerald-600 rounded-xl text-sm font-semibold text-white transition-colors"
                      >
                        <svg className="w-4 h-4" viewBox="0 0 24 24">
                          ...
                          <path fill="white" fillOpacity=".75" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
