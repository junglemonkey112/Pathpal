...
export default function ConsultantCard({ counsellor, matchScore, reasons = [] }: ConsultantCardProps) {
  return (
    <Link
      ...
      <div className="flex gap-4 md:gap-6">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <div className="flex items-center gap-2 md:gap-3 mb-1">
                ...
                <span className="text-base">{counsellor.countryFlag}</span>
                <span className="px-2 py-0.5 bg-slate-100 text-slate-600 text-xs rounded-full">{counsellor.year}</span>
              </div>
              <p className="text-slate-600">{counsellor.school}</p>
              <p className="text-sm text-slate-500">{counsellor.major}</p>
            </div>
            <div className="text-right flex-shrink-0">
              <p className="text-xl md:text-2xl font-bold text-slate-900">${counsellor.services[0].price}</p>
              <p className="text-slate-500 text-sm">/{counsellor.services[0].duration}min</p>
            </div>
          </div>

          <p className="text-slate-600 mt-2 md:mt-3 line-clamp-2 text-sm md:text-base">{counsellor.bio}</p>
          ...
          <div className="flex flex-wrap gap-2 mt-3 md:mt-4">
            {counsellor.specialties.slice(0, 4).map((specialty) => (
          ...
          <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-100">
            <div className="flex items-center gap-3 md:gap-4">
              <div className="flex items-center gap-1">
                ...
                <span className="font-semibold text-slate-900 text-sm md:text-base">{counsellor.rating}</span>
                <span className="text-slate-500 text-sm">({counsellor.reviewCount})</span>
              </div>
              <div className="flex items-center gap-1 text-emerald-600">
                <CheckCircle2 className="w-4 h-4" />
                <span className="text-sm font-medium hidden md:inline">{counsellor.studentSuccess.slice(0, 2).join(", ")}</span>
              </div>
            </div>
